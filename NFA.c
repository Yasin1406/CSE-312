...
char path[101];
char str[101];
int flag,state_num,sym_num,F_num,path_index;
...
void isNFA(char states[state_num],char sym[sym_num],char final_states[F_num],char transition_list[state_num][sym_num][state_num],int transition_num[state_num][sym_num],char cur_state,int str_index,int path_index){
    ...
    for(i=0;i<transition_num[state_index][sym_index]&&flag!=1;i++){
        ...
        printf("-----------------\n");
        printf("Current state: %c\n",states[state_index]);
     //   printf("Current elemnet: %c\n",str[str_index]);
        printf("Current symbol: %c\n",sym[sym_index]);
        printf("Transitioning to: %c\n",transition_list[state_index][sym_index][i]);
        printf("-----------------\n");
    ...
    if(flag!=1){
        printf("Traversed all possible states for State-%c Symbol-%c\n",states[state_index],sym[sym_index]);
    }
